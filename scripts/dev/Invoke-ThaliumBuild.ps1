...
$GeminiScript = Join-Path $DevDir "Invoke-Gemini.ps1"
...
Write-Host "Sending to Gemini: $($Task.Substring(0,[Math]::Min(60,$Task.Length)))..." -ForegroundColor Cyan
$Result = & $GeminiScript -Prompt $Prompt
$OutDir = Split-Path -Parent $OutputFile
if ($OutDir -and -not (Test-Path $OutDir)) { New-Item -ItemType Directory -Path $OutDir -Force | Out-Null }
$Result | Out-File -FilePath $OutputFile -Encoding UTF8 -NoNewline
Write-Host "Written: $OutputFile ($(($Result -split \"`n\").Count) lines)" -ForegroundColor Green
