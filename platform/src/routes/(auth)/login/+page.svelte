<script lang="ts">
  ...
  let tab: 'magic' | 'password' | 'google' = $state('password')
  let email = $state('')
  let password = $state('')
...
<div class="min-h-screen bg-paper flex flex-col items-center justify-center px-4">
  ...
  <div class="w-full max-w-sm bg-white border border-rule/60 px-10 py-10">
    ...
    <div class="flex gap-6 border-b border-rule mb-7">
      <button class="pb-3 font-syne font-bold text-xs tracking-wide uppercase transition-colors duration-150
          {tab === 'password' ? 'text-ink border-b-2 border-ink' : 'text-ink/30 hover:text-ink/60'}"
        onclick={() => { tab = 'password'; error = '' }}>Password</button>
      <button class="pb-3 font-syne font-bold text-xs tracking-wide uppercase transition-colors duration-150
          {tab === 'magic' ? 'text-ink border-b-2 border-ink' : 'text-ink/30 hover:text-ink/60'}"
        onclick={() => { tab = 'magic'; error = ''; sent = false }}>Magic link</button>
      <button class="pb-3 font-syne font-bold text-xs tracking-wide uppercase transition-colors duration-150
          {tab === 'google' ? 'text-ink border-b-2 border-ink' : 'text-ink/30 hover:text-ink/60'}"
        onclick={() => { tab = 'google'; error = '' }}>Google</button>
    </div>
    ...
    {:else if tab === 'magic'}
      {#if sent}
        <div class="py-2">
          ...
          <button class="mt-5 font-syne text-xs text-ink/40 hover:text-ink transition-colors underline underline-offset-2"
            onclick={() => { sent = false; email = '' }}>Use a different address</button>
        </div>
      {:else}
        <div class="space-y-5">
          <div>
      ...
      <div class="space-y-4">
        ...
        <button onclick={signInWithGoogle} disabled={loading}
          class="w-full flex items-center justify-center gap-3 border border-rule bg-white
            font-syne font-bold text-xs tracking-wide uppercase text-ink py-3 rounded
